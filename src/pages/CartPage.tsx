      ...
      <main className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          {items.length > 0 && (
            <button
              onClick={clearCart}
              className="text-red-600 hover:text-red-700 font-semibold hover:bg-red-50 px-4 py-2 rounded-2xl transition-all"
            >
              Очистить всё
            </button>
          )}
        ...
        {items.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-xl p-12 text-center border border-slate-200/50">
            <div className="w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-violet-100 to-fuchsia-100 rounded-full flex items-center justify-center">
              <ShoppingCart className="w-12 h-12 text-violet-600" />
            </div>
            <h2 className="text-2xl font-bold mb-2 text-slate-800">Корзина пуста</h2>
            <p className="text-slate-600 mb-6">Добавьте товары, чтобы начать покупки</p>
            <Link
              to="/"
              className="inline-flex items-center gap-2 bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:from-violet-700 hover:to-fuchsia-700 text-white font-bold px-8 py-4 rounded-2xl transition-all hover:shadow-lg hover:shadow-violet-500/50"
            >
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-4">
              {items.map((item) => (
                <div
                  key={item.id}
                  className="bg-white rounded-3xl p-6 shadow-lg hover:shadow-xl transition-all border border-slate-200/50"
                >
                  <div className="flex gap-6">
                    <div className="w-32 h-32 flex-shrink-0 bg-slate-100 rounded-2xl overflow-hidden">
                      <img
                        src={item.image}
                        alt={item.name}
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-2xl font-black mb-4 bg-gradient-to-r from-violet-600 to-fuchsia-600 bg-clip-text text-transparent">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 bg-slate-100 rounded-2xl p-1">
                          <button
                            ...
                            className="w-10 h-10 bg-white hover:bg-violet-600 hover:text-white rounded-xl transition-all active:scale-90 flex items-center justify-center font-bold shadow-sm"
                          >
                        ...
                        <button
                          ...
                          className="text-red-600 hover:text-white hover:bg-red-600 p-3 rounded-2xl transition-all active:scale-90"
                        >
