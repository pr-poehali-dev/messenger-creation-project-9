...
export default function StoriesBar({ stories, currentUserId, onStoryClick, onCreateStory }: StoriesBarProps) {
  ...
  return (
    <div className="flex items-center gap-4 p-4 border-b border-border overflow-x-auto scrollbar-hide">
      <div className="flex flex-col items-center gap-2 min-w-[72px] cursor-pointer group" onClick={onCreateStory}>
        <div className="relative">
          ...
          <div className="absolute bottom-0 right-0 w-6 h-6 rounded-full bg-primary flex items-center justify-center border-2 border-background group-hover:scale-110 transition-transform">
      ...
      {otherStories.map((story) => (
        ...
        >
          <div className="relative">
            <div className={`w-16 h-16 rounded-full p-0.5 ${story.viewed ? 'bg-muted' : 'bg-gradient-to-tr from-primary to-accent'}`}>
              <Avatar className="w-full h-full border-2 border-background">
                <AvatarImage src="" />
                <AvatarFallback className="gradient-primary text-white font-semibold text-lg">
                  {story.username.slice(0, 2).toUpperCase()}
